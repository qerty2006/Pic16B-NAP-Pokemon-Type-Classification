...
This project displayed the importance of always being statistically rigorous when conducting reserach, specifically showcasing what can happen with a flawed dataset or a flawed pipeline. If we didn't repeat trials we may new have noticed the high variance between different splits, or the anomalously successful trial where the stars aligned for a very accurate model. Visualization also proved to be an important part of analysing results as without it we would have not realized Efficient-NetB0's biases towards greens and blues. 
