...
from dataset import TYPES, _build_index, gen_stratified_split, rgba_to_rgb
...
def load_image_flat(path):
    """Load one sprite as a flat normalized float array."""
...
def load_all_images(index):
    """Load all sprites in parallel. Order matches the index list."""
...
def get_metrics(y_true, y_pred):
    """Return accuracy, macro F1, precision, and recall for single-label predictions."""
    return {
...
def save_mistake_examples(model_name, y_true_int, y_pred, test_paths, y_true_multihot, n=24):
    ...
    <h2 style="padding:12px">{model_name} - {len(mistakes)} mistakes out of {n_test} test
